<template>
    ...
    >
      <v-list class="pa-1">
        ...
        <v-list-tile avatar>
          ...
          <v-list-tile-content v-if="$store.state.isTenantLoggedIn">
            <v-list-tile-title>
              <p class="body-1">{{$store.state.tenant.email}}</p>
            </v-list-tile-title>
          </v-list-tile-content>
          <v-list-tile-action>
    ...
    <v-toolbar color="primary" dark tabs>
      ...
      <v-toolbar-title>Hob | Tenant</v-toolbar-title>
      ...
      <v-menu :nudge-bottom="50" light auto>
        ...
        <v-list v-if="$store.state.isTenantLoggedIn">
          <v-list-tile>
            <v-list-tile-title class="body-1">{{$store.state.tenant.email}}</v-list-tile-title>
          </v-list-tile>
...
export default {
    ...
    methods: {
      logout () {
        this.$store.dispatch('setTenantToken', null)
        this.$store.dispatch('setTenant', null)
        this.$router.push({
          name: 'home'
        })
      },
