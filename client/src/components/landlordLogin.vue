...
export default {
  ...
  methods: {
    async login () {
      try {
       const response = await AuthenticationService.loginlandlord({
          email: this.email,
          password: this.password
        })
        this.$store.dispatch('setLandlordToken', response.data.token)
        this.$store.dispatch('setLandlord', response.data.landlord)
        this.$router.push({
          name:'landlord-home'
        })
      } catch (error) {
    ...
    handleClickGetAuth(){
      ...
      .then(authCode => {
        // On success
        return this.feedback = AuthenticationService.loginlandlordgoogle({ 
